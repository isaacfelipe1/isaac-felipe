'use client';

import React from 'react';
import Image from 'next/image';
import Link from 'next/link';
import { Roboto_Mono } from 'next/font/google';
const releaseVersion = 'v1.0.1';
const robotoMono = Roboto_Mono({
  variable: '--font-roboto-mono',
  subsets: ['latin'],
  weight: ['400', '700'],
});
export default function Perfil() {
  return (
    <div className="relative flex flex-col min-h-screen  ">
      <div className="relative flex-1 flex flex-col md:flex-row">
        <div className="relative w-full h-screen md:w-1/2 md:h-auto">
          <Image
            src="/perfii_potfolio.jpg"
            alt="Foto de perfil"
            layout="fill"
            className="object-cover object-top brightness-[0.4] md:brightness-100"
            priority
          />
        </div>
        <div
          className="absolute top-0 left-0 w-full h-full flex flex-col items-center justify-center text-center text-[#F3EFE0] px-6 z-10
        md:relative md:w-1/2 lg:w-3/4 md:h-auto md:items-start md:justify-start md:text-left md:text-[#1A202C] md:bg-transparent md:p-6 md:pl-24 md:pt-50"
        >
          <div className="space-y-4">
            <h1
              className={`text-4xl md:text-5xl font-bold font-cursive ${robotoMono.variable}`}
            >
              Isaac Felipe
            </h1>
            <Link
              href="/timeline"
              className={`block text-lg md:text-xl hover:underline font-semibold ${robotoMono.variable}`}
            >
              Desenvolvedor de Software
            </Link>
            <Link
              href="/skills"
              className={`block text-lg md:text-xl hover:underline font-semibold ${robotoMono.variable}`}
            >
              Minhas Skills
            </Link>
            <div className="flex justify-center md:justify-start space-x-4 mt-4">
              <a
                href="https://github.com/isaacfelipe1"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="GitHub"
                className="text-[#F3EFE0] md:text-[#E9C46A] hover:text-[#D9A066]"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-6 w-6 md:h-8 md:w-8"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 
            11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416
            -.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729
            1.205.084 1.839 1.237 1.839 1.237
            1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604
            -2.665-.305-5.467-1.334-5.467-5.931
            0-1.311.469-2.381 1.236-3.221
            -.124-.303-.535-1.524.117-3.176
            0 0 1.008-.322 3.301 1.23
            .957-.266 1.983-.399 3.003-.404
            1.02.005 2.047.138 3.006.404
            2.291-1.552 3.297-1.23 3.297-1.23
            .653 1.653.242 2.874.118 3.176
            .77.84 1.235 1.911 1.235 3.221
            0 4.609-2.807 5.624-5.479 5.921
            .43.372.823 1.102.823 2.222v3.293
            c0 .319.192.694.801.576
            4.765-1.589 8.199-6.086 8.199-11.386
            0-6.627-5.373-12-12-12z"
                  />
                </svg>
              </a>

              <a
                href="https://www.linkedin.com/in/isaac-felipe/"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="LinkedIn"
                className="text-[#F3EFE0] md:text-[#E9C46A] hover:text-[#D9A066]"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-6 w-6 md:h-8 md:w-8"
                  fill="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
                    d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 
            2.761 2.239 5 5 5h14c2.762 0 5-2.239 
            5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268
            c-.966 0-1.75-.79-1.75-1.764s.784-1.764 
            1.75-1.764 1.75.79 1.75 1.764-.783 
            1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 
            0v5.604h-3v-11h3v1.765c1.396-2.586 
            7-2.777 7 2.476v6.759z"
                  />
                </svg>
              </a>
            </div>
          </div>
        </div>
      </div>
      <footer className="w-full bg-[#1A202C] text-[#F3EFE0] text-center py-4">
        <p>© {new Date().getFullYear()} I. Felipe- {releaseVersion} </p>
      </footer>
    </div>
  );
}
